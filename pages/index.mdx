# __Introducing Riffy__
**Riffy** is a pro lavalink client. It is designed to be simple and easy to use, with a focus on stability and more features.

## __Features__
- Supports lavalink v3 and v4 (v4 is still in beta)
- Supports all popular music platforms, including youtube, soundcloud, to use music platforms like spotify and more... make sure you have [Lavasrc](https://github.com/topi314/LavaSrc) plugin in your lavalink server (our plugin is still in beta)
- Autoplay support for youtube, soundcloud and spotify.
- Supports discord.js, eris & more.. (adjust as needed)

## __Resources__

#### Discords
- Discord.js : https://discord.js.org/
- Eris : https://abal.moe/Eris/

#### Lavalink

- Lavalink : https://github.com/lavalink-devs/Lavalink
- Lavasrc : https://github.com/topi314/LavaSrc

## __Team__
🟪 **Elitex**
- Github: [@Elitex](https://github.com/Elitex07)
- Discord: @elitex

🟥 **FlameFace**
- Github: [@FlameFace](https://github.com/flam3face)
- Discord: @flameface

🟦 **UnschooledGamer**
- Github: [@UnschooledGamer](https://github.com/UnschooledGamer)
- Discord: @unschooledgamer